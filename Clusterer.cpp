...
void TLDEDA001::Clusterer::ClusterImages()
{
    ...
    for (int i = 0; i < 10; i++)
    {
        for (int j = 0; j < 10; j++)
        {
            imagefeatures.push_back(TLDEDA001::ImageFeature(filenames[i] + to_string(j + 1), this->images[count]));
            count++;
        }
    }
    ...
    {
        this->clusters.push_back(TLDEDA001::Cluster(imagefeatures[randomIndexes[i]]));
    }
